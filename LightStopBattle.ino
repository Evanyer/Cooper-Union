...
unsigned long stateTimer = 0;     // Tracks duration of game states
unsigned long lastMoveTime = 0;   // Tracks track LED movement
int moveSpeed = 150;              // Light loop speed (ms)


int currentLed = 0;               // <-- RESTORED: The active light tracker
...
void loop() {
 ...
 switch (currentState) {
   ...
   case STARTUP:
     // Keep track completely dark for 1.5 seconds at boot.
     // Buttons are completely ignored here, letting voltage stabilize.
     for (int i = 0; i < numLeds; i++) {
       digitalWrite(ledPins[i], LOW);
     }
   ...
   case ROUND_OVER:
     ...
     if (millis() - stateTimer > 1000) {
       if (p1Score >= 3 || p2Score >= 3) {
         ...
         // Go right back to playing from the start line
         currentState = PLAYING;
         currentLed = 0;
         digitalWrite(ledPins[currentLed], HIGH);
         lastMoveTime = millis();
